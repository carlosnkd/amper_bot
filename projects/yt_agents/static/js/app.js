// Mounted at /yt-agents (see backend/app.py in the amper_bot host repo), so
// API calls need the prefix -- same pattern as Coddy's own API_BASE.
const API_BASE = '/yt-agents';
const state = {
    conversations: [],
...
function cacheElements() {
    ...
    els.welcomeOverlay = document.getElementById('welcomeOverlay');
    els.welcomeOkBtn = document.getElementById('welcomeOkBtn');
}
...
function bindEvents() {
    ...
    els.welcomeOkBtn.addEventListener('click', dismissWelcome);
}
...
function appendMessage(role, content, timestamp, sqlQuery) {
    const safeContent =
  ...
  `;
    if (role !== 'user' && sqlQuery) {
        card.appendChild(buildQueryDisclosure(sqlQuery));
    }
...
function buildQueryDisclosure(sqlQuery) {
    const wrap = document.createElement('div');
    wrap.className = 'query-disclosure';

    const toggle = document.createElement('button');
    toggle.type = 'button';
    toggle.className = 'query-toggle-btn';
    toggle.textContent = '🔍 View query';

    const block = document.createElement('div');
    block.className = 'query-block hidden';
    block.innerHTML = `
    <p class="query-block-hint">This is the exact query the agent ran against the database.</p>
    <pre><code>${escapeHtml(sqlQuery)}</code></pre>
  `;

    toggle.addEventListener('click', () => {
        const isHidden = block.classList.toggle('hidden');
        toggle.textContent = isHidden ? '🔍 View query' : '🔼 Hide query';
    });
